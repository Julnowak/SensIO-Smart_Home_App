...
model {
  // Priors (match your original prior distributions)
  ...
  for (n in 1:N) {
    ...
    y[n] ~ lognormal(mu, sigma);  // Log-normal likelihood
  }
}

generated quantities {
  vector[N] y_sim;  // Posterior predictive replicates
  vector[N] log_lik;
  ...
  for (n in 1:N) {
    ...
    // Replicate data with same truncation as prior
