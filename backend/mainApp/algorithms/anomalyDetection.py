...
def detect_anomaly(data):
    ...
    last = (IF.index[-1] in IF[IF['isolation_forest']==1]) | (data.index[-1] in data[IQR])
    lastM = (data.index[-1] in IF[IF['isolation_forest']==1]) & (data.index[-1] in data[IQR])
    consensus = matcher(
        IF['isolation_forest'] == 1,
        IQR
    )
    result = {"low_all": IF, "medium_all": consensus, "lastLow": IF['isolation_forest'].iloc[-1], "lastMedium": consensus.iloc[-1]}
